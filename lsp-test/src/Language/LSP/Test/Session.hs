...
sendMessage msg = do
  ...
  liftIO 
    $ catch @IOException (B.hPut h (addHeader $ encode msg)) 
    $ \e -> putStrLn "IO Exception: sendMessage:" 
      >> print e
      >> putStrLn "The message failed to send:"
      >> putStrLn (B.unpack . encodePretty $ msg)
